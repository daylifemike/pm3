(ns re-frame.middleware
  (:require
    ...
    [re-frame.utils :refer [info warn log group groupEnd error]]
...
(defn debug-monitor
  "Lighter event monitoring middleware, skips the diff and just logs"
  [handler]
  (fn debug-monitor-middleware [db v]
...
(defn debug
  ...
  (fn debug-handler
    ...
    (let [new-db  (handler db v)
      ...
      (when (and (or (first diff) (second diff))
                 (not (undefined? js/console.table)))
        (js/console.table (clj->js {:before (first diff)
                                    :after  (second diff)})))
      (groupEnd)
      new-db)))
...
(defn side-effect
  "Middleware which discards the results of the handler. Handler is
  assumed to be purely side-effecting."
  [handler]
  (fn side-effect-handler
    [app-db event-vec]
    (let [db (if (satisfies? IReactiveAtom app-db) @app-db app-db)]
      (handler db event-vec)
      db)))
...
(defn after-changed
  "Like `after` but only runs when `db` has changed from its previous value"
  ([f] (after-changed not= f))
  ([raw-pred f]
   (fn after-changed-middleware [handler]
     (fn after-changed-handler [db v]
       (let [new-db (handler db v)
             pred (if (set? raw-pred)
                    #(not= (select-keys db raw-pred) (select-keys new-db raw-pred))
                    raw-pred)]
         (if (pred db new-db)
           (f new-db v))
         new-db)))))
